...
#include "DarkCameraMode_ThirdPerson.h"

#include "Dark/Camera/DarkCameraAssistInterface.h"
#include "GameFramework/CameraBlockingVolume.h"
#include "GameFramework/Character.h"
#include "Curves/CurveVector.h"
#include "Engine/Canvas.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(DarkCameraMode_ThirdPerson)


namespace DarkCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UDarkCameraMode_ThirdPerson::UDarkCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FDarkPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FDarkPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FDarkPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FDarkPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FDarkPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FDarkPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FDarkPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UDarkCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UDarkCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UDarkCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IDarkCameraAssistInterface* TargetControllerAssist = Cast<IDarkCameraAssistInterface>(TargetController);

	IDarkCameraAssistInterface* TargetActorAssist = Cast<IDarkCameraAssistInterface>(TargetActor);
	...
	IDarkCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IDarkCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IDarkCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IDarkCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UDarkCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
	...
	//@Eric TODO: IDarkCameraTarget.GetIgnoredActorsForCameraPenetration();
	...
	{
		FDarkPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(DarkCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UDarkCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UDarkCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UDarkCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
